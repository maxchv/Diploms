...
if ($page>=1) {
	...
	MVC\Helpers\Html::actionLink("selectedcategory", "<font size=\"5\"><< </font>", "Admin", 1, ["class" => ""]);
	...
	MVC\Helpers\Html::actionLink("selectedcategory", "<font size=\"5\"><</font>", "Admin", $page, ["class" => ""]);
}
...
for ($j = 1; $j<$pages; $j++) {
    ...
    if ($j>=$start && $j<=$end) {
    	...
    	if ($j==($page+1)){
    		MVC\Helpers\Html::actionLink("selectedcategory", "<font size=\"5\" color=\"black\">".$j." </font>", "Admin",$j, ["class" => ""]);
    	}
        
        // Ссылки на остальные страницы
    	else {
    		MVC\Helpers\Html::actionLink("selectedcategory", " "."<font size=\"5\">".$j." </font>"." ", "Admin", $j, ["class" => ""]);
    	}
    }
}
...
if ($j>$page && ($page+2)<$j) {
    ...
    MVC\Helpers\Html::actionLink("selectedcategory", "<font size=\"5\">></font>", "Admin", $page+2, ["class" => ""]);
    // Так как у нас $j = количество страниц + 1, то теперь 
    // уменьшаем его на единицу и получаем ссылку на последнюю страницу
    MVC\Helpers\Html::actionLink("selectedcategory", "<font size=\"5\"> >></font>", "Admin", $j-1, ["class" => ""]);
}

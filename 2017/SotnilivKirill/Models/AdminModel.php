...
class AdminModel extends BaseModel{
		...
		public function adminpanel($id = null){
			$this->viewModel->set('pageTitle', 'Администрирование');
			return $this->viewModel;
		}
		public function article($id = null){
			...
			if (!empty($_POST['comment'])){
				if (strlen($_POST['comment'])>300){
					$commenterror =  "<font size=\"3\" color=\"red\">Длина комментария не должна превышать 300 символов!</font>";
				}
				else{
					$text = $_POST['comment'];
		...
		public function checkarticle($id = null){
			$this->viewModel->set('pageTitle', 'Проверить статью');
			return $this->viewModel;
		}
		...
		public function deletearticle($id = null){
			...
			$articleRow = mysqli_fetch_array($selectedArticle);
				...
				if($result==TRUE){
					$state = "<font size=\"6\" color=\"green\">Удаление выполнено успешно!</font><br /><b>Перейти на <a href='/Admin/index'>главную страницу</a></b>";
				}
				else{
					$state = "<font size=\"6\" color=\"red\">Возникла ошибка при удалении</font><br /><b>Перейти на <a href='/Admin/index'>главную страницу</a></b>";
				}
		...
		public function deletecomment($id = null){
			...
			$result = mysqli_query($db, "DELETE FROM comments WHERE Id='$id' ");
				if ($result==TRUE){
					header('Location: /Admin/article/'.$commentRow['ArticleId']);
				}
				else{
					echo "<font size=\"6\" color=\"red\">Возникла ошибка при удалении комментария</font><br /><b>Перейти на <a href='/Admin/index'>главную страницу</a></b>";
				}
